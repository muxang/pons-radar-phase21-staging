...
impl WebRepository {
    ...
    pub async fn dashboard(&self) -> Result<Value, sqlx::Error> {
        ...
        let feed: Vec<Value> = sqlx::query_scalar(r#"SELECT jsonb_build_object('seq',seq,'type',event_type,'event_effective_at',COALESCE((payload->>'event_effective_at')::timestamptz,(payload->>'block_time')::timestamptz,created_at),'classification_source',COALESCE(payload->>'classification_source',payload->>'classification_origin'),'realtime_alert_eligible',COALESCE((payload->>'realtime_alert_eligible')::boolean,false),'data',payload) FROM event_outbox WHERE event_type IN('token.launched','smart_trade.buy_confirmed','smart_trade.sell_confirmed','smart_trade.buy_backfilled','smart_trade.sell_backfilled','position.open','position.add','position.reduce','position.close','signal.watch','signal.strong_watch','signal.high_priority','signal.cooling','signal.distribution') ORDER BY seq DESC LIMIT 80"#).fetch_all(&self.pool).await?;
        Ok(
            json!({"high_priority":high,"strong_watch":strong,"launches":launches,"recent_smart_money":smart,"live_feed":feed}),
        )
    }
